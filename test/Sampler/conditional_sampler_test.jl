...
@testset "PSDModel conditional sampler" begin
    ...
    Y = SequentialTransportMaps.cond_sample(cond_sampler, [[rand() * 2 - 1] for _=1:1000]; threading=false)
...
@testset "Conditional SelfReinforcedSampler" begin
    @testset "ML 2D" begin
        ...
        ref_map = SequentialTransportMaps.ReferenceMaps.GaussianReference{2, Float64}(2.5)
        ...
        for x in range(-4.0, 4.0, length=100)
            @test abs(SequentialTransportMaps.marg_pdf(sra, [x]) - f_marg(x)) < 0.2
        end
    end
end