@testset "Scaling Reference" begin
    ...
    sra = SelfReinforcedSampler(
        ...
        model,
        1, :Chi2,
        SequentialTransportMaps.ScalingReference{2}([-5.0, 3.0], [-3.0, 10.0]);
        N_sample=500,
    )
    x = SequentialTransportMaps.sample_reference(sra)
    @test -5.0 ≤ x[1] ≤ -3.0
    @test 3.0 ≤ x[2] ≤ 10.0

    N = 20
    X = SequentialTransportMaps.sample_reference(sra, N)
    @test length(X) == N
    @test length(X[1]) == 2
    for X_i in X
        @test -5.0 ≤ X_i[1] ≤ -3.0
...
@testset "Gaussian Reference" begin
    ...
    sra = SelfReinforcedSampler(
        ...
        model,
        1, :Chi2,
        SequentialTransportMaps.GaussianReference{2, Float64}(1.0);
        N_sample=500,
    )
...
@testset "Algebraic Reference" begin
    ...
    sra = SelfReinforcedSampler(
        ...
        model,
        1, :Chi2,
        SequentialTransportMaps.AlgebraicReference{2, Float64}();
        N_sample=1000,
