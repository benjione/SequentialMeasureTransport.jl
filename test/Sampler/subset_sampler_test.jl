...
@testset "Single Projection" begin
    ...
    pdf_func = SMT.pushforward(proj_map, x->1.0)
    vec1 = pdf_func.(rng)
...
@testset "Single conditional projection" begin
    ...
    for i=1:100
        x = rand(2)
        @test isapprox(x, SMT.marginal_pullback(proj_map, SMT.marginal_pushforward(proj_map, x)), atol=1e-9)
    end

    ## for the marginal
    pb_pf_func = SMT.marginal_pullback(proj_map, SMT.marginal_pushforward(proj_map, x->1.0))
    for i=1:100
        x = rand(2)
        @test isapprox(pb_pf_func(x), 1.0, atol=1e-9)
    end
    ...
    rng = [[x, y] for x in rand(20), y in rand(20)]
    rng = reshape(rng, length(rng))
    marginal_pdf_func = SMT.marginal_pushforward(proj_map, x->1.0)
    vec1 = marginal_pdf_func.(rng)
...
@testset "Conditional from data" begin
    ...
    cond_neg_log_likelihood = (1/N) * sum(SMT.conditional_logpdf(sra_sub, x[3:3], x[1:2]) for x in eachcol(X))
    ...
    for i=1:100
        x = rand(2)
        @test isapprox(x, SMT.marginal_pullback(sra_sub, SMT.marginal_pushforward(sra_sub, x)), atol=1e-9)
    end

    ## test that marginal pushforward is pullback
    pb_pf_func = SMT.marginal_pullback(sra_sub, SMT.marginal_pushforward(sra_sub, x->1.0))
    for i=1:100
        x = rand(2)
        @test isapprox(pb_pf_func(x), 1.0, atol=1e-9)
    end
    ...
    for k=1:5
        ...
        for i=1:100
            y = rand(1)
            y_pf = SMT.conditional_pushforward(sra_sub, y, x)
            @test isapprox(y, SMT.conditional_pullback(sra_sub, y_pf, x), atol=1e-9)
        end

        pb_pf_func = SMT.conditional_pullback(sra_sub, SMT.conditional_pushforward(sra_sub, x->1.0, x), x)
        for i=1:100
            y = rand()
...
@testset "Marginal mapping with unused dimension" begin
    @testset "Simple" begin
        ...
        # compare pdfs
        f_app = SMT.pushforward(smp, x->1.0)
        f_app_marg = SMT.pushforward(smp_marg, x->1.0)
