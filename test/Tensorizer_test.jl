...
@testset "Downward closed tensorizer" begin
    @testset "2D creation" begin
        ...
        @test issetequal(t.index_list, [[1, 1], [1, 2], [2, 1], 
                               [1, 3], [2, 2], [3, 1]])
        @test issetequal(t.M_inner, [[1,3], [2, 2], [3, 1]])
    end

    @testset "2D proposals" begin
        t = DownwardClosedTensorizer(2, 2)
        prop = next_index_proposals(t)
        @test issetequal(prop, [[4, 1], [3, 2], [2, 3], 
                               [1, 4]])
    end
    ...
    @testset "2D add index" begin
        ...
        @test issetequal(t.index_list, [[1, 1], [1, 2], [2, 1], 
                               [1, 3], [2, 2], [3, 1], [4, 1]])
        @test issetequal(t.M_inner, [[1,3], [2, 2], [3, 1],[4, 1]])
    end

    @testset "3D creation" begin
        t = DownwardClosedTensorizer(3, 2)
        @test issetequal(t.index_list, [[1, 1, 1], [1, 1, 2], [1, 2, 1], 
                               [2, 1, 1], [1, 2, 2], [2, 1, 2], 
                               [2, 2, 1], [1, 1, 3], 
                               [1, 3, 1], [3, 1, 1]])
        @test issetequal(t.M_inner, [[1, 1, 3], [2, 2, 1], [1,2,2],
                        [2, 1, 2],
                        [1, 3, 1], [3, 1, 1]])
    end
    ...
    @testset "3D proposals" begin
        t = DownwardClosedTensorizer(3, 2)
        prop = next_index_proposals(t)
        @test issetequal(prop, [[1,1,4], [1,2,3], [1,3,2], [1,4,1], 
                               [2,1,3], [2,2,2], [2,3,1], 
                               [3,1,2], [3,2,1], [4,1,1]])
    end
end